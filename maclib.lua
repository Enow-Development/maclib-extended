...
function MacLib:Window(Settings)
	...
	function WindowFunctions:TabGroup()
		...
		function SectionFunctions:Tab(Settings)
			...
			-- Create columnsContainer to wrap left and right frames
			...
			columnsContainerUIListLayout.Padding = UDim.new(0, 15)
			columnsContainerUIListLayout.FillDirection = Enum.FillDirection.Horizontal
			columnsContainerUIListLayout.SortOrder = Enum.SortOrder.LayoutOrder
			columnsContainerUIListLayout.Parent = columnsContainer
			...
			right.Parent = columnsContainer

			columnsContainer.Parent = elementsScrolling

			-- Create full container for full-width sections
			local full = Instance.new("Frame")
			full.Name = "Full"
			full.AutomaticSize = Enum.AutomaticSize.Y
			full.BackgroundTransparency = 1
			full.BorderColor3 = Color3.fromRGB(0, 0, 0)
			full.BorderSizePixel = 0
			full.Size = UDim2.fromScale(1, 0)

			local fullUIListLayout = Instance.new("UIListLayout")
			fullUIListLayout.Name = "FullUIListLayout"
			fullUIListLayout.Padding = UDim.new(0, 15)
			fullUIListLayout.SortOrder = Enum.SortOrder.LayoutOrder
			fullUIListLayout.Parent = full
			...
			function TabFunctions:Section(Settings)
				...
				-- Determine parent container based on Side value with fallback to "Left"
				local parent
				if Settings.Side == "Full" then
					parent = full
